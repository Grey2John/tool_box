...
class PointSegEvaluation:
    def __init__(self, txt_path, ground_pcd, max_obs_time=150):
        """ 对应一个bag的数据分割结果估计 """
        self.test_data = DL.PointDataLoader(txt_path)
        SP = self.test_data.read_txt_list_points(upper_times=max_obs_time+1) # xyz, first label, obs
        labeled_points = generate_truth_labeled_rgb_pcd(ground_pcd, "", '', if_gen_pcd=0)
        self.labeled_points = point_sample_labeling(labeled_points, SP)  # [xyz, truth, first label, obs]
        self.num_class = 3
    ...
    def filter_process(self):
        eval_result = {}
        for i in self.labeled_points:  # i [xyz, truth, first label, obs]
    ...
    def first_label_rate(self, save_path1, save_path2):
        point_count = 0
        count = np.zeros(self.num_class ** 2)
...
if __name__ == "__main__":
    file_num = 11
    bag_txt_path = "F:\earth_rosbag\data\\test3\\r3live_4pixel\\bag{}.txt".format(file_num)
    ground_pcd = "F:\earth_rosbag\data\\test3\labeled_pcd\\origin{}.pcd".format(file_num)
    pe = PointSegEvaluation(bag_txt_path, ground_pcd)
    pe.filter_process()
    pe.first_label_rate("F:\earth_rosbag\data\\test3\evaluation\\bag{}_1.txt".format(file_num),
                        "F:\earth_rosbag\data\\test3\evaluation\\bag{}_2.txt".format(file_num))
    pe._one_time_result_filer(4, "F:\earth_rosbag\data\\test3\evaluation\\bag{}_1.txt".format(file_num), if_save=1) # 1是one
    pe._less_time_result_filer(4, "F:\earth_rosbag\data\\test3\evaluation\\bag{}_2.txt".format(file_num), if_save=1)
